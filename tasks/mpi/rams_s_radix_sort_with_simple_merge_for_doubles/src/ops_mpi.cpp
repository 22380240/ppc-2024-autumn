...
bool rams_s_radix_sort_with_simple_merge_for_doubles_mpi::TestMPITaskParallel::run() {
  ...
  boost::mpi::scatterv(world, input.data(), sendcounts, displs, local_input.data(), local_input.size(), 0);

  const size_t radix = 8;
  const size_t histogram_size = 1 << radix;
  const size_t bits_per_item = sizeof(double) * 8;
  ...
  auto get_histogram_value = [&](size_t histogram_index, double item) -> auto & {
    ...
    return histograms[histogram_index]
                     [((double_internal ^ (-(double_internal >> (bits_per_item - 1)) | (1ul << (bits_per_item - 1)))) >>
  ...
  boost::mpi::gatherv(world, local_result.data(), local_result.size(), input.data(), sendcounts, displs, 0);
